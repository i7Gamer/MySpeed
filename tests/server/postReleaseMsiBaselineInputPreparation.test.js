...
describe("post-release MSI baseline input preparation", () => {
    it("retains the exact candidate fixture and harness runtime as a closed 33/34-file subtree", () => {
        for (const includeWal of [false, true]) {
            ...
            try {
                ...
                assert.equal(result.files.length, includeWal ? 34 : 33);
                ...
                /*
                 * ost-cli is part of the inventory now. The v1.6.1 fixture itself contains none,
                 * which is precisely why that release can no longer be run through this path.
                 */
                assert.equal(result.files.some(file => file.relativePath.includes("ost-cli")), true);
