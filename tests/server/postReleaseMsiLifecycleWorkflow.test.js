...
/*
 * Both sealed controllers this job runs rebuild their own context from the runner's environment,
 * and the source commit reaches them through MYSPEED_SOURCE_SHA alone - neither reads github.sha.
 * The request they are handed takes the same commit from EXPECTED_SOURCE_SHA, so a job that
 * declares one of the two names and not the other cannot agree with itself, and every dispatch
 * stops at the first controller it runs. The sibling CPU-floor workflow declares both. This runs
 * the real derivation against the environment this job actually declares rather than reading the
 * YAML for a name, so it fails the way the runner would.
 */
describe("the MSI lifecycle execute environment reaches the sealed controllers", () => {
    const COMMIT = "1ac257dd6e9ffa67c67743f5b0a004cf5438ad07";
    const RUN_ID = "34986357039";
    const RUN_ATTEMPT = "1";
    const NONCE = "0123456789abcdef0123456789abcdef";
    const IMAGE_VERSION = "20260901.1.0";
    const HOSTED_RUNTIME = {platform: "linux", architecture: "x64"};

    /* What GitHub itself places in the environment of the ubuntu-24.04 runner this job names. */
    const RUNNER_ENVIRONMENT = {GITHUB_ACTIONS: "true", CI: "true", RUNNER_OS: "Linux",
        RUNNER_ARCH: "X64", RUNNER_ENVIRONMENT: "github-hosted", GITHUB_REPOSITORY: "i7Gamer/MySpeed",
        GITHUB_SHA: COMMIT, GITHUB_RUN_ID: RUN_ID, GITHUB_RUN_ATTEMPT: RUN_ATTEMPT,
        ImageOS: "ubuntu24", ImageVersion: IMAGE_VERSION};

    /* Only the expressions the derivation depends on; every other value stays the literal text. */
    const EXPRESSIONS = new Map([["${{ github.sha }}", COMMIT], ["${{ github.run_id }}", RUN_ID],
        ["${{ github.run_attempt }}", RUN_ATTEMPT]]);

    const expand = block => Object.fromEntries(Object.entries(block ?? {}).map(([name, value]) =>
        [name, EXPRESSIONS.get(String(value).trim()) ?? String(value)]));

    const executeEnvironment = () => ({...RUNNER_ENVIRONMENT, ...expand(WORKFLOW.env),
        ...expand(WORKFLOW.jobs.execute.env)});

    it("derives the same source commit the request context is built from", () => {
        const environment = executeEnvironment();

        const actual = deriveActualHostedContext(NONCE, environment, HOSTED_RUNTIME);

        assert.equal(actual.sourceSha, COMMIT);
        assert.equal(actual.sourceSha, environment.EXPECTED_SOURCE_SHA,
