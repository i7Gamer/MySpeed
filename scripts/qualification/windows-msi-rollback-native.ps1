...
public static class MyspeedMsiRollbackNative
{
    ...
    internal const uint OwnerSecurityInformation=1,GroupSecurityInformation=2,DaclSecurityInformation=4;
    ...
    [DllImport("advapi32.dll",SetLastError=true)]
    static extern bool SetKernelObjectSecurity(SafeFileHandle handle,uint information,byte[] descriptor);
    ...
    public sealed class DirectorySecurityLease:IDisposable{
        ...
        public void ApplyDeny(){AssertIdentity();DenyAttempted=true;RawSecurityDescriptor raw=new RawSecurityDescriptor(original,0);RawAcl old=raw.DiscretionaryAcl;if(old==null)throw new InvalidDataException("Directory DACL is absent");SecurityIdentifier everyone=new SecurityIdentifier(WellKnownSidType.WorldSid,null);RawAcl next=new RawAcl(old.Revision,old.Count+1);next.InsertAce(0,new CommonAce(AceFlags.None,AceQualifier.AccessDenied,FileAddFile,everyone,false,null));for(int i=0;i<old.Count;i++)next.InsertAce(i+1,old[i]);RawSecurityDescriptor changed=new RawSecurityDescriptor(raw.ControlFlags,raw.Owner,raw.Group,raw.SystemAcl,next);byte[] bytes=new byte[changed.BinaryLength];changed.GetBinaryForm(bytes,0);if(!SetKernelObjectSecurity(handle,DaclSecurityInformation,bytes))throw Error("SetKernelObjectSecurity deny");AssertIdentity();RawSecurityDescriptor observed=new RawSecurityDescriptor(Security(handle),0);CommonAce first=observed.DiscretionaryAcl[0] as CommonAce;if((observed.ControlFlags&ControlFlags.DiscretionaryAclProtected)==0||first==null||first.AceQualifier!=AceQualifier.AccessDenied||first.AccessMask!=FileAddFile||first.AceFlags!=AceFlags.None||!everyone.Equals(first.SecurityIdentifier))throw new InvalidDataException("Directory deny ACE differs");DenyActive=true;}
        public void ProveCreateDenied(string target){AssertIdentity();if(!String.Equals(Path.GetDirectoryName(Path.GetFullPath(target)),expectedPath,StringComparison.OrdinalIgnoreCase))throw new InvalidDataException("Denied target is outside directory");try{using(FileStream stream=new FileStream(target,FileMode.CreateNew,FileAccess.Write,FileShare.None)){}File.Delete(target);throw new InvalidDataException("Directory create was not denied");}catch(UnauthorizedAccessException){}}
        public void RestoreOriginalSecurity(){if(!DenyAttempted){AssertIdentity();if(!Equal(Security(handle),original))throw new InvalidDataException("Directory security changed without deny");Restored=true;return;}if(!SetKernelObjectSecurity(handle,DaclSecurityInformation,original))throw Error("SetKernelObjectSecurity restore");AssertIdentity();if(!Equal(Security(handle),original))throw new InvalidDataException("Directory security did not restore exactly");DenyActive=false;Restored=true;}
    ...
    public sealed class CallbackState{
        readonly string predecessorCode,targetPath,fileToken,directoryToken;readonly long payloadBytes;readonly DirectorySecurityLease lease;
        readonly List<RecordSnapshot> records=new List<RecordSnapshot>();string currentAction;
        public string Failure{get;private set;}public string CleanupFailure{get;private set;}public bool RemovalStartSeen{get;private set;}public bool RemovalProductSeen{get;private set;}
        ...
        public bool SecurityRestoredBeforeCancel{get;private set;}public int ErrorResponse{get;private set;}public RecordSnapshot[] Records{get{return records.ToArray();}}
        public CallbackState(string predecessor,string target,string expectedFileToken,string expectedDirectoryToken,long expectedBytes,DirectorySecurityLease security){predecessorCode=predecessor;targetPath=Path.GetFullPath(target);fileToken=expectedFileToken;directoryToken=expectedDirectoryToken;payloadBytes=expectedBytes;lease=security;}
        ...
        int Observe(RecordSnapshot record){if(records.Count>=MaximumRecords)throw new InvalidDataException("Callback record count exceeded");records.Add(record);if(SecurityRestoredBeforeCancel)return ResponseOk;uint messageClass=record.MessageTypeCode&MessageClassMask;if(messageClass==MessageActionStart){if(record.Fields.Length<1)throw new InvalidDataException("ACTIONSTART record is empty");currentAction=record.Fields[0];if(currentAction=="RemoveExistingProducts")RemovalStartSeen=true;if(currentAction=="InstallFiles"){if(DenyInjectionAttempted)throw new InvalidDataException("Deny injection was already attempted");DenyInjectionAttempted=true;if(!RemovalProductSeen||File.Exists(targetPath))throw new InvalidDataException("InstallFiles began before predecessor removal proof");lease.ApplyDeny();lease.ProveCreateDenied(targetPath);InstallFilesSeen=true;}return ResponseOk;}if(messageClass==MessageActionData){if(currentAction=="RemoveExistingProducts"&&record.Fields.Length>=1&&record.Fields[0]==predecessorCode)RemovalProductSeen=true;if(currentAction=="InstallFiles"&&record.Fields.Length>=9&&record.Fields[0]==fileToken&&record.Fields[5]==payloadBytes.ToString(System.Globalization.CultureInfo.InvariantCulture)&&record.Fields[8]==directoryToken)InstallDataSeen=true;return ResponseOk;}if(messageClass==MessageError){int targetMatches=0;for(int index=1;index<record.Fields.Length;index++)if(MatchesTarget(record.Fields[index]))targetMatches++;if(!InstallFilesSeen||!InstallDataSeen||!lease.DenyActive||record.Field1Integer!=ErrorWritingToFile||targetMatches!=1)throw new InvalidDataException("ERROR record is not bound to the denied target");lease.RestoreOriginalSecurity();SecurityRestoredBeforeCancel=true;ErrorSeen=true;uint style=record.MessageTypeCode&MessageStyleMask;if(style==ErrorRetryCancelStyle)ErrorResponse=ResponseCancel;else if(style==ErrorAbortRetryIgnoreStyle)ErrorResponse=ResponseAbort;else throw new InvalidDataException("ERROR response style is unsupported");return ErrorResponse;}return ResponseOk;}
        public int Invoke(IntPtr context,uint messageType,uint recordHandle){try{return Observe(SnapshotRecord(messageType,recordHandle));}catch{if(Failure==null)Failure="callback-record-or-observer-failure";try{lease.RestoreOriginalSecurity();}catch{if(CleanupFailure==null)CleanupFailure="callback-security-restore-failure";}return CallbackFailureReturn;}}
    }
