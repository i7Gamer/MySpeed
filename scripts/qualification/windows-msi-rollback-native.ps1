...
$script:AcceptedInstallReturns = @(1602, 1603)
...
function Get-MyspeedRollbackNativeContract {
    [pscustomobject]@{
        ...
        acceptedInstallReturns = @($script:AcceptedInstallReturns)
...
{
    internal const uint ErrorSuccess=0,ErrorMoreData=234,ErrorInsufficientBuffer=122,ErrorInstallUserExit=1602,ErrorInstallFailure=1603;
    ...
    internal const int InstallStateUnknown=-1,InstallStateDefault=5,InstallLevelDefault=0,InstallStateAbsent=2,ErrorCreatingDestinationFile=1310,ExpectedSystemError=0;
    internal const int ExpectedErrorFieldCount=3,ExpectedErrorCodeFieldIndex=0,ExpectedSystemErrorFieldIndex=1,ExpectedTargetFieldIndex=2;
    ...
    internal const uint ErrorRetryCancelStyle=5;
    internal const int ResponseOk=1,ResponseCancel=2,CallbackFailureReturn=-1;
    ...
    public sealed class DirectorySecurityLease:IDisposable{
        ...
        public void ProveCreateDenied(string target){DiagnosticStage="create-denied-identity";AssertIdentity();DiagnosticStage="create-denied-target";if(!String.Equals(Path.GetDirectoryName(Path.GetFullPath(target)),expectedPath,StringComparison.OrdinalIgnoreCase))throw new InvalidDataException("Denied target is outside directory");DiagnosticStage="create-denied-attempt";try{using(FileStream stream=new FileStream(target,FileMode.CreateNew,FileAccess.Write,FileShare.None)){}}catch(UnauthorizedAccessException){DiagnosticStage=null;return;}DiagnosticStage="create-denied-cleanup";try{File.Delete(target);}finally{throw new InvalidDataException("Directory create was not denied");}}
    ...
    public sealed class CallbackState{
        ...
        public bool InstallFilesSeen{get;private set;}public bool DenyInjectionAttempted{get;private set;}public bool InstallDataSeen{get;private set;}public bool ErrorSeen{get;private set;}public int InstallFilesStartCount{get;private set;}public int DenyReproofCount{get;private set;}
        public bool SecurityRestoredBeforeCancel{get;private set;}public int ErrorResponse{get;private set;}public int ErrorCode{get;private set;}public int ErrorSystemCode{get;private set;}public bool InstallContextBalanced{get{return installContext.Balanced;}}public RecordSnapshot[] Records{get{return records.ToArray();}}
    ...
    public static CalibrationResult RunCalibration(string predecessorMsi,string candidateMsi,string predecessorCode,string candidateCode,string targetDirectory,string targetFile,string marker,string markerValue,string predecessorHash,long payloadBytes,string logPath){
        ...
        try{
            ...
            candidateInstallAttempted=true;try{output.CandidateInstallReturn=MsiInstallProductW(candidateMsi,"REBOOT=ReallySuppress");}finally{GC.KeepAlive(callback);}
            output.PredecessorAfterRollback=MsiQueryProductStateW(predecessorCode);output.CandidateAfterRollback=MsiQueryProductStateW(candidateCode);output.predecessorPayloadRestored=File.Exists(targetFile);output.predecessorPayloadSha256=output.predecessorPayloadRestored?Sha256File(targetFile):null;output.candidateAbsent=output.CandidateAfterRollback==InstallStateUnknown;output.sentinelPreserved=File.Exists(marker);output.sentinelBytesPreserved=output.sentinelPreserved&&Encoding.ASCII.GetString(File.ReadAllBytes(marker))==markerValue+"\n";output.postReturnStateObserved=true;
            if(state.Failure!=null||state.CleanupFailure!=null||!state.InstallContextBalanced||!state.RemovalStartSeen||!state.RemovalProductSeen||!state.InstallFilesSeen||!state.InstallDataSeen||!state.ErrorSeen||!state.SecurityRestoredBeforeCancel)throw new InvalidOperationException("Callback calibration proof is incomplete");if(output.CandidateInstallReturn!=ErrorInstallUserExit&&output.CandidateInstallReturn!=ErrorInstallFailure)throw new InvalidOperationException("Candidate install return is not a reviewed rollback outcome");if(output.PredecessorAfterRollback!=InstallStateDefault||!output.predecessorPayloadRestored||output.predecessorPayloadSha256!=predecessorHash||!output.candidateAbsent||!output.sentinelBytesPreserved)throw new InvalidOperationException("Rollback state differs");
